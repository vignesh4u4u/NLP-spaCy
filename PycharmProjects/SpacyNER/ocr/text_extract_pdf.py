...
class PDFTextExtrator:
    ...
    def extract_text(self, file, selected_options):
        ...
        if (
            "addresses" in selected_options
            or "owner_address" in selected_options
            or "tenant_address" in selected_options
        ):
            symbols_to_remove = ['▪', '■', '►', '✦', '◦', '✶', '❖', '✪', '➤', "•"]
            cleaned_text = text
            ...
            if all_addresses:
                ...
                if "addresses" in selected_options:
                    data["addresses"] = {
                        f"address_{idx}": address
                        for idx, address in enumerate(unique_addresses, start=1)
                    }
        ...
        if "names" in selected_options:
            ...
            formatted_names = filtered_names
            formatted_text = " ".join(formatted_names)
            doc1 = PDFTextExtrator.nlp(formatted_text)
            final_name =[]
            for ent1 in doc1.ents:
                if ent1.label_ in ["PERSON","ORG"]:
                    final_name.append(ent1.text)
            all_extracted1 = set(final_name)
            final_filter_names = {
                f"name_{idx}": name for idx, name in enumerate(all_extracted1, start=1)
            }
            data["names"] = final_filter_names
            data["name_count"] = len(final_filter_names)
