...
def image_text_conversion():
    if request.method=="POST":
        ...
        if (key =="Text" or key=="text"):
            text_data=text
        data=None
        if (key == "Date" or key == "DATE" or key == "date" or key == "invoice date") and (pattern == re.compile(r'\d{1,2}[/-]\d{1,2}[/-]\d{2,4}') or pattern == "\d{1,2}-(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)-\d{2,4}" or pattern == "?:jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?) \d{1,2}, \d{4}"):
        ...
        if key =="Table":
            ...
            print(json_data)
        if  not data:
            return render_template("single.html", message="No relevant information found")

    return render_template("single.html", **locals())
